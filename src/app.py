...
class App:
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="📦 计算器", font=("Arial", 18, "bold"),
        ...
        # 根据工具名称添加特定功能
        if "timer" in "calculator".lower() or "clock" in "calculator".lower():
            self.setup_timer_ui(main)
        elif "random" in "calculator".lower() or "dice" in "calculator".lower() or "coin" in "calculator".lower():
            self.setup_random_ui(main)
        elif "note" in "calculator".lower() or "todo" in "calculator".lower():
    ...
    def generate_random(self):
        if "dice" in "calculator".lower():
            result = random.randint(1, 6)
            self.result_label.config(text=f"🎲 {result}")
        elif "coin" in "calculator".lower():
